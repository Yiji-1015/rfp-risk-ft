...
`--retrieval global`은 앵커가 고정이라 앵커 블록까지 캐시된다(결정 29).

| 인출 방식 | 캐시되는 구간 | 전수 1,024건 배치 입력비 |
|---|---|---:|
| `stratified` (기본) | 기본 블록만 | 약 6,912원 |
| `global` | 기본 블록 + 앵커 블록 | 약 2,619원 |
...
전수 1,024건은 **단일 경로가 아니다.** Chunk 1 재실행 배치를 취소했기 때문이다.
...
5. **캐시 절감은 인출 방식에 따라 다르다.** 고정 앵커(`--retrieval global`)만 앵커 블록이
   캐시되며, 층화·유사도는 앵커가 건마다 달라져 캐시되지 않는다(결정 29).
