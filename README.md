...
| 과제 | 최고 구성 | macro F1 |
|---|---|---:|
| **통상수용 vs 검토필요** (2분류) | word+char TF-IDF + Logistic | **0.788** |
| 통상수용 / 견적반영 / 계약·질의검토 (3분류) | word+char + 파인튜닝 base·large 다수결 | 0.649 (seed 30쌍 평균, 23/30 우세, 최대 0.677) |
| 〃 단일 모델 | word+char TF-IDF + Logistic | 0.638 |
| 〃 최저 기준 | Dummy (최빈 클래스) | 0.354 |

*924건 통합 OOF 기준. 동결 앵커 100건은 라벨 생성 때 예시로 쓰였으므로 평가에서 제외한다.*

**실무 질문("이 조항, 그냥 받아도 되나")에 맞춘 2분류는 0.788로 쓸 만하다.** 3분류가
0.64에서 멈추는 이유는 [`docs/guides/02`](docs/guides/02-model-comparison.md)와 아래
"3분류는 왜 0.64에서 멈추나"에 있다.

### 무엇을 시도했고 무엇이 안 통했나

E5 임베딩, 한국어 의존구문, 요구사항 유형·구조·수치 feature, 양식 표현 마스킹,
경량 인코더 파인튜닝(small·base·large, KoELECTRA, KoBigBird 1024), 보조 헤드
(blocker·원가·난이도) 다중 과제 파인튜닝, 경계 겨냥 feature(불릿 max-pooling·blocker 유형
타깃), `견적 vs 계약` 전용 2분류 — **어느 것도 단순 문자 TF-IDF를 넘지 못했다.**
파인튜닝은 seed 범위가 0.038로 TF-IDF와의 차이보다 커서 단독으로는 우열을 말할 수 없다.
보조 헤드는 base의 seed 편차를 0.038→0.008로 줄였지만 평균은 그대로였다.

유일하게 통한 것은 **계열을 섞는 것**이다. TF-IDF와 파인튜닝은 오답 294건과 282건 중
180건만 겹친다. 같은 계열끼리 묶은 기존 soft voting은 +0.001이고, 파인튜닝 base·large를
섞은 다수결은 seed 조합 30쌍 평균 **+0.011**, 23쌍에서 우세하다. 0.677은 그중 운 좋은
seed 쌍이며, 어느 seed를 뽑아도 되는 값은 0.649다.

### 3분류는 왜 0.64에서 멈추나

오답의 3분의 1이 `견적반영`과 `계약·질의검토` 사이 **한 경계**에 몰려 있다. 두 라벨을
합치면 0.788이 되고, 합치는 방법이 학습이든 사후 변환이든 결과가 같다.

아홉 개 모델이 전원 같은 방향으로 틀린 28건을 열어보면 원인이 보인다. 판정에 필요한 것은
어휘가 아니라 **쓰이지 않은 조항**(수치 목표가 없어서 견적)이거나 **문서 밖 지식**
(`폐쇄망`이면 상용 도구 조달이 막힌다)이다. 계열을 바꾸고 용량을 세 배로 키우고 입력을
바꿔도 이 28건은 움직이지 않았다. 자세한 것은
[`reports/current/v4/boundary_agreement.md`](reports/current/v4/boundary_agreement.md).

### 라벨은 얼마나 재현되는가

모델이 멈춘 자리에서 정답 라벨 자체를 다시 쟀다. 모델과 라벨이 어긋난 오답 296건을 같은
라벨러(Sonnet 5, 프롬프트 v5)로 zero-shot 2회 다시 라벨링했다.

| | 같은 조건 2회 일치 | 동결 라벨(few-shot)과 일치 | 3표 다수결이 동결과 다름 |
|---|---:|---:|---:|
| 견적↔계약 무작위 표본 100 | 0.94 | 0.85 | 12% |
| 통상↔계약 오답 94 | 0.90 | – | 21% |
| 통상↔견적 오답 102 | 0.85 | – | 16% |

라벨러는 자기 답을 90~94% 다시 내지만 동결 라벨과는 85%만 맞는다. 차이의 절반은 층화
앵커가 넣은 노이즈다. **모델이 반대하는 자리가 라벨이 흔들리는 자리**이기도 해서(무작위
12% 대 오답 표본 16~21%), 모델 불일치는 라벨 감사 표본으로 쓸 만하다. 다만 뒤집힌 라벨은
모델 쪽이 아니라 가운데(`견적반영`)로 가므로, 재라벨링으로 회수되는 모델 오답은 2% 안팎이다.

읽어보니 흔들림은 무작위가 아니라 **규칙이 정하지 않은 두 자리**였다. 기준 수행사가 무엇을
기본 공수로 갖는가, 그리고 관행 조항 안의 무엇이 blocker인가. 그래서 프롬프트 v6는 예외
목록을 모두 빼고 blocker를 다시 정의했다 — "확인하면 좋은 조건"이 아니라 **"이대로 받으면
견적으로 덮을 수 없는 손해가 생기는 조항"**. 이 정의로 전수 1,024건 × 3회 zero-shot 배치를
제출해 두었고, 결과는 v4의 개선판이 아니라 다른 을의 눈높이로 적어 나란히 비교한다.
[`notebooks/16_label_recheck.ipynb`](notebooks/16_label_recheck.ipynb),
[`reports/current/v4/prompt_v6_recheck.md`](reports/current/v4/prompt_v6_recheck.md).
...
| 13 | `13_label_boundary.ipynb` | 오답이 몰린 경계, 2분류 사후 접기와 정식 학습 비교 |
| 14 | `14_text_masking.ipynb` | 양식 표현 마스킹 3규칙 ablation |
| 15 | `15_finetuning.ipynb` | 경량 인코더 파인튜닝 곡선·seed 편차·앙상블 |
| 16 | `16_label_recheck.ipynb` | 라벨러 자기일치, 모델 불일치 감사, 프롬프트 v6 비교 |
...
각 단계는 **복잡하게 만든 값을 했는가**에 답하기 위해 존재한다. 네 칸을 모두 채웠고,
답은 "3번과 4번은 값을 못 했다"이다.

```
1. DummyClassifier              최저 기준 (최빈 클래스만 찍기)          [완료] 0.219
2. TF-IDF + Logistic / SVM      주 기준선 — 문자 n-gram               [완료] 0.614
3. 사전학습 문장 임베딩 + ML      의미 표현이 단어 겹침보다 나은가        [완료] 0.533~0.606
4. 경량 한국어 인코더 파인튜닝    GPU 예산 대비 추가 이득이 있는가        [완료] 0.540~0.617
```

*fold 평균 기준. 파인튜닝은 gcube GPU에서 10 fold를 여섯 번 돌린 값이며 실비 약 1,000원.*
...
- **v6b 전수 라벨 비교** — blocker를 다시 정의한 프롬프트로 만든 3표 다수결 라벨이
  도착하면, 같은 모델·같은 분할로 v4와 나란히 학습해 라벨 정의가 downstream에 얼마나
  전달되는지 본다. 앵커 없이 만든 라벨이라 100건 평가 제외도 사라진다.
- **새 RFP 외부 평가** — 지금 모든 값은 같은 924건에서 나왔다. 앙상블 확정은 처음 보는
  문서 하나에서 한다.
- 하이퍼파라미터 inner LODO와 파인튜닝 다중 헤드는 시험했고 접었다(각각 기준선 이하,
  평균 변화 없음).
